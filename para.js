...
function buildSVG() {
  ...
  ch1.forEach((ch, i) => {
    ...
    fillTextsA.push(
      `<text x="${mx}" y="${baseY1}" font-family="${f1}" font-size="${s1}" fill="${c1}" mask="url(#m${id})">${esc(
        ch
      )}</text>`
    );
...
function init() {
  ...
  // モーダル閉じる（×/背景/Esc）
  document.getElementById("colorPanelClose")?.addEventListener("click", (e) => {
    e.preventDefault();
    closeColorPanel();
  });
